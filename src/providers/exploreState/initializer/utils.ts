...
import { buildQuery } from "../../exploreStateSync/hooks/UseMetaCommands/utils";
...
/**
 * Initializes entity query parameters.
 ...
 * @returns Entity query parameters.
 */
function initEntities(
  ...
  return Object.keys(entityPageState).reduce((acc, entityPath) => {
    return {
      ...
      [entityPath]: {
        query: buildQuery({
          catalog: decodedCatalogParam,
          entityListType: entityPath,
          ff: decodedFeatureFlagParam,
          filter: entityListType === entityPath ? filterState : [],
        }),
      },
    };
  }, {} as EntitiesContext);
