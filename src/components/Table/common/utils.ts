 ...
 * @param exploreStatePageSize - Explore state page size.
 * @param listView - List view config.
 * @returns table state pagination.
 */
export function getTableStatePagination(
  exploreStatePageSize: number,
  listView?: ListViewConfig
): PaginationState {
  const { disablePagination } = listView || {};
  const pageIndex = 0;
  const pageSize = exploreStatePageSize; //disablePagination ? MAX_SAFE_INTEGER : exploreStatePageSize;
  return {
    pageIndex,
    pageSize,
  };
