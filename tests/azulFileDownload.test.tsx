...
describe("AzulFileDownload", () => {
  ...
  describe("download functionality", () => {
    ...
    test("should mark the same button busy while the request is in flight", async () => {
      render(<AzulFileDownload {...TRACKING_PARAMETERS} url={URL} />);
      const buttonEl = getButtonById(AZUL_FILE_REQUEST_DOWNLOAD_TEST_ID);
      fireEvent.click(buttonEl);
      await waitFor(() => {
      ...
      // off, so IconButton suppresses the pointer affordance itself: without it
      // the spinner keeps cursor: pointer and the theme's hover / active
    ...
    test("should ignore clicks while the request is in flight", async () => {
      render(<AzulFileDownload {...TRACKING_PARAMETERS} url={URL} />);
      const buttonEl = getButtonById(AZUL_FILE_REQUEST_DOWNLOAD_TEST_ID);
      fireEvent.click(buttonEl);
      await waitFor(() => {
